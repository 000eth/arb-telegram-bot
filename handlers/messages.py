...
def register_message_handlers(dp: Dispatcher):
    ...
    # ВАЖНО: Сначала регистрируем обработчик свободного текста (для ручного ввода)
    # Он должен быть ПЕРВЫМ, но с проверкой pending_action
    
    @dp.message()
    async def handle_free_text(message: Message):
        """Обработчик всех текстовых сообщений - должен быть первым"""
        s = get_user_settings(message.from_user.id)

        # Если есть pending_action, обрабатываем как ручной ввод
        if s.pending_action:
